...
public class DynamicOverviewContentExtension extends OverviewContentExtension {
    ...
    public DynamicOverviewContentExtension(String contentPath) {
        super();
        ...
        if (StringUtils.isBlank(contentPath)) {
            if (globalContext.swaggerLocation == null) {
                if (logger.isWarnEnabled())
                    logger.warn("DynamicOverviewContentExtension disabled > Can't set default contentPath from null swaggerLocation. You have to explicitly configure the content path.");
            } else {
